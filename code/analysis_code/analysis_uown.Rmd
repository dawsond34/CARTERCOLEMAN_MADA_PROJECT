...
```{r}
#Create a location and save linear model for E. coli (cfu) in the MIDO train data.
MIDO_ECFU_Con_fit_location <- here::here("results","MIDO_ECFU_Con_fit.rds")
saveRDS(ecfu_con_fit, file = MIDO_ECFU_Con_fit_location)

#Create a location and save linear model for E. coli (cfu) in the MIDO test data.
MIDO_ECFU_Con_fit_test_location <- here::here("results","MIDO_ECFU_Con_fit_test.rds")
saveRDS(ecfu_con_fit_test, file = MIDO_ECFU_Con_fit_test_location)
```
</br>
#### Save significant Test and Train RMSE to the results folder.
```{r}
#Create a location and save RMSE for E. coli (cfu) in the MIDO train data.
MIDO_ECFU_Con_RMSE_location <- here::here("results","MIDO_ECFU_Con_RMSE.rds")
saveRDS(ecfu_con_aug, file = MIDO_ECFU_Con_RMSE_location)

#Create a location and save linear RMSE for E. coli (cfu) in the MIDO test data.
MIDO_ECFU_Con_RMSE_test_location <- here::here("results","MIDO_ECFU_Con_RMSE_test.rds")
saveRDS(ecfu_con_aug_test, file = MIDO_ECFU_Con_RMSE_test_location)
```
...
Null_Met
```
_RMSE = 6.63, with a standard deviation of 0.13. This will serve as our check to test our models against latter on.
